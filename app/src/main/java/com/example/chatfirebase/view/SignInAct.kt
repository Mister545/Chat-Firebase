...
class SignInAct : AppCompatActivity() {
    ...
    var reg = true
    val firebase = FirebaseRepositoryImpl()
    ...
    private fun setRegisterUserStateInBD(user: FirebaseUser, name: String, randomCodeForTodoChat: String) {
        ...
        firebaseRepositoryImpl.setChat(
            Chat(participants = mutableListOf(user.uid),
            messages = arrayListOf(Message(user.uid, "Hi I am your todo chat", "", time = DataTimeHelper().getIsoUtcFormat())),
