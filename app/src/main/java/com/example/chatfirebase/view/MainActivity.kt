...
import com.example.chatfirebase.viewModel.ChatsViewModel
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val updateChatsB = findViewById<ImageView>(R.id.updateChats)
        navController.addOnDestinationChangedListener { controller, destination, arguments ->
            when (destination.id) {
                R.id.nav_chat -> {
                    fabNewChat.visibility = View.VISIBLE
                    updateChatsB.visibility = View.VISIBLE
                    fabNewChat.setImageResource(R.drawable.baseline_add_24)
                }
                R.id.nav_new_group -> {
                    updateChatsB.visibility = View.GONE
                    fabNewChat.visibility = View.VISIBLE
    ...
    private fun clickOnSavedListener(){
        FirebaseRepositoryImpl().getUser(FirebaseAuth.getInstance().uid!!){
